...
public sealed class PullingSystem : EntitySystem
{
    ...
    private void HandlePullStopped(EntityUid uid, HandsComponent component, PullStoppedMessage args)
    {
        ...
        {
            if (!TryComp(held, out VirtualItemComponent? virtualItem) || GetRelayedEntity(virtualItem.BlockingEntity) != args.PulledUid)
                continue;
    ...
    private void OnVirtualItemDeleted(Entity<PullerComponent> ent, ref VirtualItemDeletedEvent args)
    {
        ...
        if (ent.Comp.Pulling is not { } pulling || pulling != GetRelayedEntity(args.BlockingEntity))
            return;

        if (TryComp(pulling, out PullableComponent? pullableComp))
            TryStopPull(pulling, pullableComp, ent.Owner);
    }
    ...
    private void PopupGrabRetakeResult(EntityUid pullerUid, EntityUid currentPuller, EntityUid pullableUid, bool success)
    {
        if (!_netManager.IsServer)
            return;

        var player = Identity.Entity(pullerUid, EntityManager);
        var current = Identity.Entity(currentPuller, EntityManager);
        var pulled = Identity.Entity(pullableUid, EntityManager);
        var baseKey = success ? "popup-grab-retake-success" : "popup-grab-retake-fail";

        _popup.PopupPredicted(Loc.GetString(baseKey,
                ("puller", current),
                ("pulled", pulled)),
            pullerUid,
            pullerUid,
            PopupType.MediumCaution);
        _popup.PopupPredicted(Loc.GetString(baseKey + "-puller",
                ("puller", player),
                ("pulled", pulled)),
            currentPuller,
            currentPuller,
        ...
        bool force = false)
    {
        ...
        if (pullableComp.Puller != null)
        {
            ...
            {
                PopupGrabRetakeResult(pullerUid, currentPuller, pullableUid, false);
                return false;
            }

            PopupGrabRetakeResult(pullerUid, currentPuller, pullableUid, true);
        }
